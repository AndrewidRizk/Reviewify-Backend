...
def summarize_reviews(classified_reviews):
  positive_text = "Summarize the following to 2-3 sentences in an informative fashion, in third person, and with a general tone (don't mention names), picking key words that peopple like (Quality, Quanity, service, etc). If no reviews provided just say 'no reviews': "
  negative_text = "Summarize the following to 2-3 sentences in an informative fashion, in third person, and with a general tone (don't mention names), picking key words that peopple like (Quality, Quanity, service, etc). If no reviews provided just say 'no reviews': "

  #Concatenate positive and negative reviews to their respective strings
  for i in classified_reviews:
    if (i[1] == 'positive'):
      if len(i[0]) > 420:
         positive_text += positive_text + " " + i[0][:420]
      else:
        positive_text += positive_text + " " + i[0]
    elif (i[1] == 'negative'):
      if len(i[0]) > 420:
         negative_text += negative_text + " " + i[0][:420]
      else:
